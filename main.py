...
def main():
    root = tk.Tk()
    root.title("Járműszegmentáló Alkalmazás")
...
def initialize_control_vars():
    controls = {
        'noise_controls': [("Zaj intenzitás", Scale, 'noise_intensity', {'from_': 0, 'to': 0.1, 'resolution': 0.01}),
                           ("Zaj típusa", OptionMenu, 'noise_type', {'options': ['gaussian', 'salt_pepper']})],
        'preprocessing_controls': [("Kontraszt korlátozás", Scale, 'contrast_clip_limit', {'from_': 3.0, 'to': 5.0, 'resolution': 0.1}),
                                   ("Csempe rács méret", Scale, 'tile_grid_size', {'from_': 2, 'to': 8, 'resolution': 1}),
                                   ("Szín csatorna", OptionMenu, 'color_channel', {'options': ['BGR', 'HSV', 'YCrCb']})],
        'segmentation_controls': [("Szegmentálási terület arány", Scale, 'segmentation_area_ratio', {'from_': 0.0005, 'to': 0.01, 'resolution': 0.0005}),
                                  ("Max terület arány", Scale, 'max_area_ratio', {'from_': 0.2, 'to': 0.5, 'resolution': 0.05}),
                                  ("Minimális arányszám", Scale, 'min_aspect_ratio', {'from_': 0.2, 'to': 1.0, 'resolution': 0.1}),
                                  ("Maximális arányszám", Scale, 'max_aspect_ratio', {'from_': 1.0, 'to': 5.5, 'resolution': 0.1}),
                                  ("Minimális tömörség", Scale, 'min_solidity', {'from_': 0.4, 'to': 1.0, 'resolution': 0.1}),
                                  ("Csúcsszám küszöb", Scale, 'vertex_threshold', {'from_': 4, 'to': 20, 'resolution': 1})],
        'postprocessing_controls': [("Utófeldolgozási kernel méret", Scale, 'postprocess_kernel_size', {'from_': 5, 'to': 9, 'resolution': 1}),
                                    ("Morfologikus műveletek", Scale, 'morphology_operations', {'from_': 1, 'to': 7, 'resolution': 1})]
    }
    return {
        'control_vars': {
...
def setup_gui(root, control_vars, randomize_vars, image):
    ...
    titles_top = ["Eredeti", "Zajos", "Előfeldolgozott"]
    titles_bottom = ["Szegmentált", "Utófeldolgozott"]
    ...
    Button(control_frame, text="Összes randomizálása", command=lambda: randomize_all(control_vars, randomize_vars, lambda: update_processing(control_vars, image_labels, image, randomize_vars))).grid(row=1, column=0, columnspan=4)
    return image_labels
...
def setup_controls(frames, control_vars, randomize_vars, update_function, image_labels, image):
    ...
    for section, controls in control_vars['controls'].items():
        ...
        for row_index, (label_text, widget, var_name, kwargs) in enumerate(controls):
            ...
            Checkbutton(frame, text="Randomizálás", variable=randomize_vars[var_name]).grid(row=row_index, column=2)
        Button(frame, text="Szakasz randomizálása", command=lambda fr=section: set_random_values(control_vars, randomize_vars, lambda: update_processing(control_vars, image_labels, image, randomize_vars), fr)).grid(row=len(controls), column=0, columnspan=3)
